...
interface AudioSeatGridProps {
  ...
  userAccountToUid?: Map<string, number>;
...
export function AudioSeatGrid({
  ...
  const renderOccupiedSeat = (occupant: AudioSeatOccupant) => {
    ...
    const hasVideo = occupant.isVideoEnabled && isVideoRoom && (isMe || (remoteUid != null && remoteUids.has(remoteUid)));
      ...
      >
        ...
        {hasVideo ? (
          <View style={StyleSheet.absoluteFill}>
            ...
            {(!isMe && !remoteUids.has(userAccountToUid.get(occupant.userId) ?? -1)) && (
              <View style={styles.videoStreamContainer}>
                <Ionicons name="videocam" size={22} color="rgba(255,255,255,0.25)" />
                <Text style={styles.videoStreamTag}>Live Video</Text>
              </View>
            )}
        ...
        {occupant.isHost && (
          <View style={styles.hostCrownBadge}>
            <Ionicons name="crown" size={10} color="#3B2400" />
          </View>
        )}
