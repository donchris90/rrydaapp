...
export function useLiveChat(context: 'LIVE' | 'ROOM', contextId: string, options: UseLiveChatOptions = {}) {
  ...
  useEffect(() => {
    ...
    const reloadAfterReconnect = async () => {
      try {
        const history = await fetchChatHistory(context, contextId, { limit: 50 });
        if (!cancelled) setMessages((prev) => mergeMessages(history, prev));
      } catch {
        // The socket is still usable; the next reconnect or manual refresh can recover history.
      }
    };
    ...
    socket.on('connect', () => {
      ...
      // Socket.IO reconnects automatically, but events sent while the socket was
      // offline are not replayed. Re-reading the last 50 persisted messages makes
      // reconnect behavior lossless from the user's point of view.
      if (hasConnectedBefore) void reloadAfterReconnect();
    ...
    socket.on('chat:message', (raw: unknown) => {
      const message = raw && typeof raw === 'object' ? raw as Partial<ChatMessage> : null;
      if (!message || typeof message.id !== 'string' || typeof message.senderId !== 'string' || typeof message.content !== 'string') return;
    ...
    socket.on('gift:sent', (raw: unknown) => {
      const event = raw && typeof raw === 'object' ? raw as Partial<GiftEvent> : null;
      if (!event || typeof event.senderId !== 'string' || typeof event.recipientId !== 'string' || typeof event.giftId !== 'string') return;
      const normalized: GiftEvent = {
        id: typeof event.id === 'string' ? event.id : undefined,
        senderId: event.senderId,
        senderName: typeof event.senderName === 'string' ? event.senderName : null,
        senderAvatarUrl: typeof event.senderAvatarUrl === 'string' ? event.senderAvatarUrl : null,
        recipientId: event.recipientId,
        recipientName: typeof event.recipientName === 'string' ? event.recipientName : null,
        giftId: event.giftId,
        giftName: typeof event.giftName === 'string' ? event.giftName : null,
        giftIcon: typeof event.giftIcon === 'string' ? event.giftIcon : null,
        coinAmount: Number.isFinite(Number(event.coinAmount)) ? Number(event.coinAmount) : 0,
      };
      setGiftEvents((prev) => [...prev.slice(-4), normalized]);
    });

    socket.on('live:media', (raw: unknown) => {
      if (!raw || typeof raw !== 'object') return;
      onMediaRef.current?.(raw as LiveMediaMessage);
    });
    ...
    if (context === 'LIVE') {
      ...
      socket.on('live:like', (raw: unknown) => {
        const value = raw && typeof raw === 'object' ? Number((raw as any).likeCount) : NaN;
        if (!Number.isFinite(value) || value < 0) return;
        setLikeCount((prev) => Math.max(prev ?? 0, Math.floor(value)));
      });
      socket.on('live:viewer_count', (raw: unknown) => {
        const value = raw && typeof raw === 'object' ? Number((raw as any).viewerCount) : NaN;
        if (!Number.isFinite(value) || value < 0) return;
        setViewerCount(Math.floor(value));
      });
