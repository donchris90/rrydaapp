...
import { LiveViewerActionsSheet } from '../../components/LiveViewerActionsSheet';
...
export function GoLiveScreen() {
  ...
  // Solo-live daily earning target. The target is set before going live and
  // the active room shows progress as gross gift coins earned in this stream.
  const [dailyTargetCoins, setDailyTargetCoins] = useState('1000');
  ...
  const [viewerActions, setViewerActions] = useState<LiveViewer | null>(null);
  ...
  const {
    ...
    viewerCount,
    sendLike,
  } = useLiveChat('LIVE', session?.id ?? '', {
    onMedia: (m) => applyMedia(m),
    onModeration: (event) => {
      if (event.action === 'KICK' || event.action === 'BAN') {
        queryClient.invalidateQueries({ queryKey: ['live', session?.id, 'viewers'] });
      }
    },
  });
  ...
  const startMutation = useMutation({
    mutationFn: () =>
      createLiveSession({
        ...
        dailyTargetCoins: Math.max(100, Math.min(10_000_000, Number.parseInt(dailyTargetCoins || '1000', 10) || 1000)),
      }),
          ...
          <View style={styles.setupCard}>
            ...
            <View style={styles.targetHeaderRow}>
              <View style={styles.targetLabelWrap}>
                <Ionicons name="trophy-outline" size={17} color="#FFC24B" />
                <View>
                  <Text style={styles.sectionLabel}>Daily Target</Text>
                  <Text style={styles.targetHint}>Set your gift-coin goal for this solo live</Text>
                </View>
              </View>
              <View style={styles.targetInputWrap}>
                <TextInput
                  value={dailyTargetCoins}
                  onChangeText={(value) => setDailyTargetCoins(value.replace(/[^0-9]/g, '').slice(0, 8))}
                ...
                />
                <Text style={styles.targetInputSuffix}>coins</Text>
              </View>
            </View>
            <View style={styles.targetPresetsRow}>
              {[500, 1000, 5000, 10000].map((value) => (
                <Pressable
                  key={value}
                  onPress={() => setDailyTargetCoins(String(value))}
                  style={[styles.targetPreset, Number(dailyTargetCoins) === value && styles.targetPresetActive]}
                >
                  <Text style={[styles.targetPresetText, Number(dailyTargetCoins) === value && styles.targetPresetTextActive]}>
  ...
  return (
    <View style={styles.root}>
      ...
      {/* Solo-live daily target: progress is the gross gift coins earned in this live. */}
      {session.dailyTargetCoins != null && session.dailyTargetCoins > 0 && !isPkActive && (
        <Pressable
          onPress={() => {
            const current = Math.min(coinsEarned, session.dailyTargetCoins!);
            const remaining = Math.max(0, session.dailyTargetCoins! - current);
            Alert.alert(
              'Daily Target',
              current >= session.dailyTargetCoins!
                ? `You reached your ${session.dailyTargetCoins!.toLocaleString()} coin target 🎉`
                : `${current.toLocaleString()} / ${session.dailyTargetCoins!.toLocaleString()} coins\n\n${remaining.toLocaleString()} coins remaining. Keep engaging with your viewers!`,
            );
          }}
          style={[styles.dailyTargetPill, { top: insets.top + 62 }]}
          accessibilityRole="button"
          accessibilityLabel="View daily target progress"
        >
          <View style={styles.dailyTargetTopRow}>
            <View style={styles.dailyTargetTitleRow}>
              <Ionicons name="trophy" size={14} color="#FFC24B" />
              <Text style={styles.dailyTargetTitle}>Daily Target</Text>
            </View>
            <Text style={styles.dailyTargetValue}>
              {Math.min(coinsEarned, session.dailyTargetCoins).toLocaleString()} / {session.dailyTargetCoins.toLocaleString()}
            </Text>
          </View>
          <View style={styles.dailyTargetTrack}>
            <View
              style={[
                styles.dailyTargetFill,
                { width: `${Math.min(100, Math.round((coinsEarned / session.dailyTargetCoins) * 100))}%` },
              ]}
            />
          </View>
        </Pressable>
      ...
      <LiveChatOverlay
        ...
        onSenderPress={(message) => {
          // Hosts can moderate a real chat participant directly from the message.
          // Reuse the same moderation sheet used by the viewer list so every entry
          // point goes through the same backend-enforced actions.
          if (!message.senderId || message.senderId === 'system' || message.senderId === user?.id) return;

          const existingViewer = viewersQuery.data?.find((v) => v.userId === message.senderId);
          setViewerActions(
            existingViewer ?? {
              userId: message.senderId,
              displayName: message.senderName || 'Viewer',
      ...
      <ViewerPickerSheet
        ...
        onSelect={(viewer) => {
          ...
          setViewerActions(viewer);
        }}
      />
      <LiveViewerActionsSheet
        visible={!!viewerActions}
        viewer={viewerActions}
        onClose={() => setViewerActions(null)}
        onGift={() => {
          const viewer = viewerActions;
          setViewerActions(null);
          if (viewer) setGiftRecipient(viewer);
        }}
        onMute={async () => {
          const viewer = viewerActions;
          setViewerActions(null);
          ...
          try {
            await muteLiveViewer(session.id, viewer.userId);
            Alert.alert('Viewer muted', `${viewer.displayName ?? 'This viewer'} can no longer send chat messages in this live.`);
          } catch (error: any) {
            Alert.alert('Could not mute viewer', error?.response?.data?.message ?? 'Please try again.');
          }
        }}
        onKick={async () => {
          const viewer = viewerActions;
          ...
          if (!viewer || !session) return;
          try {
            await kickLiveViewer(session.id, viewer.userId);
            queryClient.invalidateQueries({ queryKey: ['live', session.id, 'viewers'] });
          } catch (error: any) {
            Alert.alert('Could not kick viewer', error?.response?.data?.message ?? 'Please try again.');
          }
        }}
        onBan={async () => {
          const viewer = viewerActions;
          ...
          if (!viewer || !session) return;
          try {
            await banLiveViewer(session.id, viewer.userId);
            queryClient.invalidateQueries({ queryKey: ['live', session.id, 'viewers'] });
          } catch (error: any) {
            Alert.alert('Could not ban viewer', error?.response?.data?.message ?? 'Please try again.');
          }
        }}
        onBlock={async () => {
          const viewer = viewerActions;
          ...
          if (!viewer) return;
          try {
            await blockUser(viewer.userId);
            if (session) {
              await kickLiveViewer(session.id, viewer.userId).catch(() => {});
              queryClient.invalidateQueries({ queryKey: ['live', session.id, 'viewers'] });
            }
            Alert.alert('User blocked', `${viewer.displayName ?? 'This user'} has been blocked.`);
          } catch (error: any) {
            Alert.alert('Could not block user', error?.response?.data?.message ?? 'Please try again.');
          }
        }}
      />
      ...
      <StreamSummaryModal
        ...
        stats={{
          ...
          targetCoins: session?.dailyTargetCoins ?? null,
          targetProgressCoins: endSummary?.giftCoins ?? coinsEarned,
...
const styles = StyleSheet.create({
  ...
  targetInputWrap: {
    ...
    backgroundColor: 'rgba(255,255,255,0.08)',
    borderWidth: 1,
    borderColor: 'rgba(255,194,75,0.45)',
    borderRadius: 11,
    paddingHorizontal: 9,
    height: 38,
  },
  ...
  targetPresetActive: {
    ...
    borderColor: 'rgba(255,194,75,0.65)',
  },
  targetPresetText: {
    color: '#B0A6D6',
    fontSize: 10,
